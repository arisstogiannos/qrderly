...
export const extractSomeItemsJob = inngest.createFunction(
  ...
  async ({ event, step }) => {
          ...
          step.run(`process-image-${index}`, async () => {
            ...
            const work = (async () => {
              ...
              if (names?.length && uploadedFile) {
                ...
                const { menuItems: remainingItems } = safeParse(revivedResponse || '');
    ...
    if ('data' in createdCategories && createdCategories.data) {
      const menuItemsToCreate = allMenuItems.filter((item) => !existingItems.includes(item.name));

...
export default function CreateMenuBtn({ session }: { session: Session | null }) {
  ...
  const paidUnPublishedSub = subscriptions.find(
    (sub) =>
      ...
      (!sub.businessId || !business.find((b) => b.id === sub.businessId)?.menu?.published),
  );

  return !paidUnPublishedSub ? (
    ...
    <Modal
      ...
      trigger={
        <Button className="lg:bg-foreground ml-2 p-4 mr-2  text-lg">{t('createMenu')}</Button>
      }
    >

...
export default function Product({ title, desc, steps, videoPath }: thisProps) {
  return (
    <div id={title.replaceAll(" ","-")} className="grid grid-cols-1 grid-rows-[auto_auto] xl:grid-cols-2 gap-6 md:gap-16 xl:gap-20">
      <div className="space-y-10">
        <div className="space-y-2">
          <h2 className=" text-3xl md:text-4xl xl:text-3xl font-medium">{title}</h2>
          <p className="text-base md:text-2xl xl:text-xl">{desc}</p>
        </div>
        <div className="space-y-4">
          <h3 className="font-medium text-4xl md:text-3xl xl:text-2xl">How it works</h3>
        ...
        <Button className="text-lg md:text-2xl xl:text-xl rounded-full p-6 px-10 gap-x-4" asChild><Link href={"/pricing"}>Get Started for Free <ArrowRight className="size-6"/></Link></Button>
      </div>

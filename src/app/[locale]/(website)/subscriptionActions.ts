...
export async function createFreeSubscription(
    ...
    const sub = user.subscriptions.find((s)=>s.billing ==="FREETRIAL" && s.product === productMap[product])
  ...
  if(!sub){
    try {
     ...
     await db.user.update({
        where: { id: user.id },
        select: { subscriptions: { orderBy: { purchasedAt: "desc" }, take: 1 } },
  
        data: {
          subscriptions: {
            upsert: {
              where: {
                businessId_product: { businessId, product: productMap[product] },
              },
  ...
  export async function createSession(priceId: string, billing: BillingType, product: Product, businessId:string ,freeSubscriptionId:string, successPageLink:string, successPageButton:string) {
    const user = (await auth())?.user;

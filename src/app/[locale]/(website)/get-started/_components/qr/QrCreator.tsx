...
export default function QrCreator({
	...
	const initialOptions: Options = business.qr?.qrOptions
		? JSON.parse(business.qr.qrOptions)
		: null;
	const qrRef = useRef<HTMLDivElement | null>(null);
	const t = useTranslations("qr settings");
	const [bgColor, setBgColor] = useState(
		initialOptions?.backgroundOptions?.color ?? "#ffffff",
	);
	const [text, setText] = useState(business.qr?.text ?? "Scan For Menu");
	const [shape, setShape] = useState<DotType>(
		initialOptions?.dotsOptions?.type ?? "square",
	);
	...
	const [dotColor, setDotColor] = useState(
		initialOptions?.dotsOptions?.color ?? "#000000",
	);

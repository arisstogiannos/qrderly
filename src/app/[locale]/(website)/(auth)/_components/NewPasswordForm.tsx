...
export function NewPasswordForm({ token }: { token: string  }) {
  const [state, newPasswordAction, isPending] = useActionState(
    newPassword,
    null
  );
  ...
  >
    <form action={newPasswordAction} className="space-y-6">
      ...
      {(state?.error || state?.errors?.token || state?.success) &&<div className="row-start-3 content-end space-y-10 pb-1">

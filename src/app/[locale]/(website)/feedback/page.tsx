...
const feedbackFormSchema = z.object({
  ...
  feedbackType: z.string().min(1, { message: "validation.type" }),
  comment: z
    .string()
    ...
    <div className="flex space-x-1">
      {[1, 2, 3, 4, 5].map((rating) => (
        <button
          ...
          type="button"
        >
          <label htmlFor={`rating-${rating}`} className="cursor-pointer p-2 rounded-full hover:bg-muted">
            <Star
              className={`h-8 w-8 ${
                hoveredRating !== null
          ...
          <input type="radio" id={`rating-${rating}`} name="rating" value={rating.toString()} className="sr-only" />
        </button>
      ))}
    </div>
  )
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-3">
            <div>
              ...
              <StarRating value={formData.rating} onChange={(value) => handleChange("rating", value)} />
          ...
          <div className="space-y-2">
            ...
            <Select value={formData.feedbackType} onValueChange={(value) => handleChange("feedbackType", value)}>
              ...
              <SelectContent>
                ...
                <SelectItem value="feature">{t("type.options.feature")}</SelectItem>
                <SelectItem value="improvement">{t("type.options.improvement")}</SelectItem>
                <SelectItem value="praise">{t("type.options.praise")}</SelectItem>
                <SelectItem value="other">{t("type.options.other")}</SelectItem>
              </SelectContent>
            </Select>
            {errors.feedbackType && <p className="text-sm font-medium text-destructive mt-1">{errors.feedbackType}</p>}
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-sm text-muted-foreground">{t("comment.charCount", { count: formData.comment.length })}</p>

  ...
  return (
    <section className="grid grid-cols-1  lg:grid-cols-[auto_auto] 2xl:grid-cols-2 gap-y-10 sm:pr-20 lg:pr-0 mt-0 lg:mt-0">
      <div className=" space-y-3 lg:max-w-sm xl:max-w-xl">
        <h2 className="font-semibold text-primary md:text-lg 2xl:text-xl">{t('title')}</h2>
        <p className="text-lg md:text-xl lg:text-2xl text-balance 2xl:text-3xl">{t('subtitle')}</p>
      </div>
        ...
        {[1, 2, 3, 4, 5].map((i) => (
          <ListItem
            ...
            desc={t.rich(`bullets.${i}.subtitle`, {
              a: (chunks) => (
                <a className="underline underline-offset-1 text-primary" href="#order-menu-form">
                  {chunks}
                </a>
              ),
            })}
...
function ListItem({ title, desc }: { title: string; desc: string | ReactNode }) {
  return (

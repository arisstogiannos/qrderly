  ...
  return (
    <section className="grid grid-rows-[auto_auto] lg:grid-cols-2 grid-cols-1  gap-20 relative w-full my-40 ">
      ...
      <div className="space-y-4 max-sm:hidden">
        ...
        <p className="text-xl font-normal  text-background/70">{t('subtitle')}</p>
      </div>
          ...
          {themes[template].slice(0, 3).map((theme, i) => {
            ...
            return (
              <label key={theme} className="cursor-pointer">
                ...
                <div
                  className={
                    'sm:h-14 sm:w-20 w-16 h-12 overflow-hidden rounded-xl grid grid-cols-3 border-4 transition-all peer-checked:scale-110 border-transparent'
                  }
                >
                  {colors.map((color) => (
...
function Setting({ children, title }: { children: React.ReactNode; title: string }) {
  return (

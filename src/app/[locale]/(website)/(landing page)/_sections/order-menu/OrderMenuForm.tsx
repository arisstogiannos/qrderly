...
const orderMenuSchema = z.object({
  ...
  comment: z.string().max(500, { message: 'validation.comment.max' }),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (formData.images?.length) {
        await uploadMultipleImagesClientSide(formData.images, `menu-orders/${formData.email}`);
      }
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-12 items-center">
      ...
      <div className="space-y-6 text-center lg:text-left 2xl:pr-20">
        <h2 className="2xl:text-6xl text-4xl font-bold ">{t('title')}</h2>
        <p className="2xl:text-2xl text-lg text-foreground/90">{t('description')}</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="qrMenu">{t('product.qrMenu')}</SelectItem>
                <SelectItem value="orderingQrMenu">{t('product.orderingQrMenu')}</SelectItem>
                <SelectItem value="selfServiceMenu">{t('product.selfServiceMenu')}</SelectItem>
              </SelectContent>
            </Select>
            {errors.product && (
              <p className="text-sm font-medium text-red-900 mt-1">{errors.product}</p>
            )}
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            {errors.email && <p className="text-sm font-medium text-red-900">{errors.email}</p>}
          </div>

          <ImageUploader images={formData.images || []} onImagesChange={handleImagesChange} t={t} />
          {errors.images && <p className="text-sm font-medium text-red-900">{errors.images}</p>}
          ...
          <div className="space-y-2">
            ...
            {errors.comment && <p className="text-sm font-medium text-red-900">{errors.comment}</p>}
          </div>
          <input
            type="text"
            name="phone"

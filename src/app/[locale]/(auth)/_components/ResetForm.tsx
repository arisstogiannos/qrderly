...
export default function ResetForm({ className, ...props }: React.ComponentProps<'div'>) {
    ...
    <FormWrapper title={t('title')} subtitle={t('subtitle')}>
      <form action={resetAction} className="min-h-[100px]">
        <div className="flex flex-col gap-6">
          ...
          <Button disabled={isPending} type="submit" className="mt-3 w-full  text-lg">
        ...
        {(state?.errors?.email || state?.success) && (
          <div className="mt-6 space-y-10 pb-1">
            ...
            {state?.success && <SuccessMessage msg={t('resetLinkSent')} />}
          </div>
        )}
      </form>
    </FormWrapper>

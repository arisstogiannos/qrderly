...
function getLocalizedName(locale: string): { name: string; shortName: string } {
  if (locale === 'el') {
    return {
      name: 'Qrderly Πίνακας Ελέγχου',
      shortName: 'Qrderly Admin',
    };
  }

  return {
    name: 'Qrderly Dashboard',
    shortName: 'Qrderly Admin',
  };
}
...
export default async function manifest({
  params,
}: ManifestParams): Promise<MetadataRoute.Manifest> {
  ...
  return {
    ...
    theme_color: '#0f172a',
    description: 'Manage your Qrderly menu, orders, and QR settings from anywhere.',
